...
Обратные зависимости отсутствуют: репозитории ничего не знают о HTTP,
обработчики — о SQL.
...
- **Целостность денег** — переводы в транзакции с `SELECT ... FOR UPDATE`,
  ...
  ограничение `CHECK (balance >= 0)` на уровне БД как второй рубеж.
...
```
P = S · i · (1+i)ⁿ / ((1+i)ⁿ − 1),   i = годовая ставка / 12 / 100
```
...
Перед реальным использованием обязательно замените `JWT_SECRET`,
`HMAC_SECRET` и `PGP_PASSPHRASE`. Смена `PGP_PASSPHRASE` сделает
ранее выпущенные карты нечитаемыми, а смена `HMAC_SECRET` — уронит проверку
их целостности.
